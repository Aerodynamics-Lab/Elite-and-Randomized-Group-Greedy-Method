function [isensors,History] = F_sensor_GEG_sub(U, pmax, Lmax)
%for debug
History = cell(2,pmax);
%end for debug    
% Lmax=50;
...
for p = 1:pmax
    ...
    if p == 1         
        objlist_all  = [];
        isensors_all = [];        
        [isensors_inc, objlist] = F_sensor_EG_inc(U, p, isensors, Lmax);
        ...
        for L=1:Lmax
            [isensors_inc, objlist] = F_sensor_EG_inc(U, p, isensors(L,:), Lmax);
            objlist_all  = [ objlist_all  ; objlist ] ;
